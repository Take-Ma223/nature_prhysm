		...
		for (NoteCounter = 0; NoteCounter <= nc[lane] - 1; NoteCounter++) {
			if ((note[lane][NoteCounter].group == NoteGroup::Single || note[lane][NoteCounter].group == NoteGroup::LongNoteStart) && note[lane][NoteCounter].color != NoteColor::K) {
				if (firstFlag == 0) {
					...
					chainBuf_16 = (double)weight_ratio_16 / pow((((double)note[lane][NoteCounter].timing_real - TimingBuf) * normalizer_16), 8);//BaseBPMの16分より短いと1からどんどん上がっていく
					chainBuf_8 = (double)weight_ratio_8 / pow((((double)note[lane][NoteCounter].timing_real - TimingBuf) * normalizer_8), 4);//BaseBPMの8分より短いと1からどんどん上がっていく
					...
					chain[lane] += chainBuf_16 > chainBuf_8 ? chainBuf_16 : chainBuf_8;//大きい方の値を足す
					TimingBuf = note[lane][NoteCounter].timing_real;
				}
			}
		}
