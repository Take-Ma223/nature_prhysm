...
void ConfigLoad(Config *config) {//Configƒtƒ@ƒCƒ‹‚Ì“Ç‚Ýž‚Ý
	...
	while (FileRead_gets(str, 256, fp) != -1) {//ˆêsŽæ“¾(\n‚Í“ü‚ç‚È‚¢)
		...
		if (wcscmp(L"USE_AI_PREDICTED_DIFFICULTY", sharp1) == 0) {
			...
			swscanf_s(sharp2, L"%[^,\n]",
				Val, _countof(Val));
			config->UseAiPredictedDifficulty = _wtoi(Val);//”’lŠi”[
		}
	...
	if (config->Vsync == TRUE) {
		//ƒfƒBƒXƒvƒŒƒC‚ÌƒŠƒtƒŒƒbƒVƒ…ƒŒ[ƒg‚ðÝ’è‚·‚é ’Êí‚Í60 ƒ‚ƒjƒ^‚É‚æ‚Á‚Ä120‚È‚Ç
		int RefreshRate;
		HDC hdc;

		hdc = GetDC(GetMainWindowHandle());	// ƒfƒoƒCƒXƒRƒ“ƒeƒLƒXƒg‚ÌŽæ“¾
		RefreshRate = GetDeviceCaps(hdc, VREFRESH);	// ƒŠƒtƒŒƒbƒVƒ…ƒŒ[ƒg‚ÌŽæ“¾
		ReleaseDC(GetMainWindowHandle(), hdc);	// ƒfƒoƒCƒXƒRƒ“ƒeƒLƒXƒg‚Ì‰ð•ú
