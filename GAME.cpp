...
#define ROOT12_2 1.0594630943592952645618252949463
	...
	const auto fastSlowCordinate = Cordinate(lane[0], judge_area + fastSlowY);
	Image I_Fast[4] = {
	Image(asset.img(fast.c_str()), &GAME_passed_time_for_UI, fastSlowCordinate, 0),
	Image(asset.img(fast.c_str()), &GAME_passed_time_for_UI, fastSlowCordinate, 0),
	Image(asset.img(fast.c_str()), &GAME_passed_time_for_UI, fastSlowCordinate, 0),
	Image(asset.img(fast.c_str()), &GAME_passed_time_for_UI, fastSlowCordinate, 0)
	};
	Image I_Slow[4] = {
	Image(asset.img(slow.c_str()), &GAME_passed_time_for_UI, fastSlowCordinate, 0),
	Image(asset.img(slow.c_str()), &GAME_passed_time_for_UI, fastSlowCordinate, 0),
	Image(asset.img(slow.c_str()), &GAME_passed_time_for_UI, fastSlowCordinate, 0),
	Image(asset.img(slow.c_str()), &GAME_passed_time_for_UI, fastSlowCordinate, 0)
	};
	...
	while (1) {
		...
		if (*debug == 1) {//デバッグモードONなら
			...
			if (Key[KEY_INPUT_F11] == 1) {//エディタ画面に移行
				...
				GAME_LOAD(song_number, difficulty, note, barline, lane, 0, &Cdiff, option, bpmchange, scrollchange, stopSequence, &hash, Music, &MusicSub, &TimeToEndScroll, &playing_time, config, 1, &score_cell_head);//score_cell_headに譜面情報をロード
		...
		*/
		...
		if (ClearFlag != 0 && str_draw_counter == 0 && c_m_draw_counter == 0 && (GAME_passed_time - cleared_time >= TimeFromEndOfGameToResult || *escape == 1)) {//クリア表示4秒後にゲームの終了(結果発表へ)
			...
			if (*debug == 1 && pitch_step != 0) {
				GAME_LOAD(song_number, difficulty, note, barline, lane, 0, &Cdiff, option, bpmchange,scrollchange, stopSequence, &hash, Music, &MusicSub, &TimeToEndScroll, &playing_time, config, 1);//ピッチが変更されているときはこの関数の終わりにピッチ1で読み込みなおして
