...
int DifficultyRadar::CalcColor(int StartTime, int EndTime, int Rainbow) {//色難易度
	...
	serachNotesBFS([&](int lane, int index) {
		if ((note[lane][index].group == NoteGroup::Single || note[lane][index].group == NoteGroup::LongNoteStart || note[lane][index].group == NoteGroup::LongNoteMiddle)) {//単ノートかLN始点か中間ノート
		...
		else if (note[lane][index].group == NoteGroup::LongNoteEnd && note[lane][index].LN_k == 1) {//黒終点の時
			ColorChangeCount += 0.5;
			k_flag = 1;
		}
