...
void SONG_SELECT(int *l_n, 
	...
	wchar_t *ope_ins[7] = {//operation instruct 操作説明 
		L"決定"
		,L"選択"
		,L"難易度変更"
		,L"リザルト表示"
	...
	while (1) {
		...
		if (flag == FLAG_SELECT_STATE) {
			...
			if (Key[KEY_INPUT_F2] == 1) {//デバッグモードONOFF
				if (ScoreShowMode == 0) {
					ScoreShowMode = 1;
					PlaySoundMem(SH_DIFFICULTY_SELECT, DX_PLAYTYPE_BACK, TRUE);
				}
		...
		oi_counter = int(0.0004*GetNowCount_d(config)) % 7;//操作説明表示用の0~6のカウンタ
...
wchar_t * announce_str(int StageCount, int PlayCount) {//通常アナウンスを返す
	...
	L"【ヒント:ゲージの種類】オプションでゲージの種類を選ぶことができます。　より厳しいゲージでのクリアを目指しましょう。",
	...
	L"【ヒント:未調査エリア】「???」と表示されている曲は未調査エリアです。　危険なエリアのため立ち入りは禁じられています。　万が一調査する場合は厳重に警戒してください。",
	...
	L"【ヒント:～～Music地方】～～Music地方は、ここから遠く離れた場所を示しています。　曲が気に入った場合はそちらに赴くのも良いでしょう。"
	};
